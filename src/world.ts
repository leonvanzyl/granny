...
import { LEVEL, GROUP, MASS } from './config';
...
export function buildWorld(physics, scene, rig, seed) {
  ...
  const doors = [];
  const anchors = [];
  const interactRefs: any = {};
  const hideSpots = [];
  let anchorId = 0;
  let pieceSeq = 0;   // one increment per place() call (one furniture piece)
  ...
  function makeDoor(axis, fixed, g) {
    ...
    handle.position.set(leafW / 2 - 0.12, 0, leafT); mesh.add(handle);
    scene.add(mesh);
  ...
  function place(builderName, roomKey, lx, lz, yaw, movable) {
    const pieceId = pieceSeq++;   // tag every collider of this piece with one id
    ...
    if (movable) {
      ...
      // collider `offset` IS the box CENTER (furniture builder convention), so the
      // body centre y is offset[1] directly — do NOT add size/2 (that floated every
      // collider half its height above the mesh).
      const centerY = c.offset[1];
      const pivot = new THREE.Group();
      pivot.add(g);
      g.position.set(0, -centerY, 0);
      g.rotation.y = yaw;
      scene.add(pivot);
      const fbody = physics.addFurnitureBox([sw, c.size[1], sd], [wx, centerY, wz], MASS[movable] || 20, pivot);
      fbody.userData.pieceId = pieceId;
      fbody.userData.pieceName = builderName;
      return pivot;
    }
    ...
    for (const c of (built.colliders || [])) {
      ...
      const [sw, sd] = rotSize(c.size[0], c.size[2]);
      // `offset` is the collider CENTER (not its base) — use it directly for the
      // body centre. (Previously added size/2, floating colliders above the mesh.)
      const pos = [wx + ox, c.offset[1], wz + oz];
    ...
    for (const ci of colInfo) {
      const sbody = physics.addStaticBox(ci.size, ci.pos);
      sbody.userData.pieceId = pieceId;
      sbody.userData.pieceName = builderName;
  ...
  place('nightstand', 'bedroom', 2.1, 0.5, 0, false); // beside the bed head (was 0.4,0.5 — clipped into the bed)
  ...
  place('nightstand', 'bedroom2', 1.4, 2.5, 0, false); // beside the bed (was 3.3,4.0 — clipped the bed corner)

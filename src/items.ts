...
import { MASS, PLAYER, GROUP, LEVEL } from './config';
import { MaterialLibrary } from './materials';
import { mulberry32, shuffleInPlace } from './util';
...
export function buildItemMesh(type) {
  ...
  if (type === 'rustyKey' || type === 'brassKey') {
    ...
    g.add(bow, shaft, bit); size = [0.103, 0.066, 0.066]; // matched to mesh bbox (lab Catalog Audit)
    ...
    g.add(handle, shaft); size = [0.2, 0.035, 0.036]; // matched to mesh bbox (lab Catalog Audit)
    ...
    // matched to mesh bbox (lab Catalog Audit) — the bolt cutter's jaw extends it
    size = type === 'boltCutter' ? [0.47, 0.065, 0.07] : [0.41, 0.065, 0.07];
    ...
    g.add(cog); size = [0.076, 0.08, 0.02]; // matched to mesh bbox (lab Catalog Audit)
...
export class Puzzle {
  ...
  know: { safeCode: boolean; keypadCode: boolean };
  locks: { l1: boolean; l2: boolean; l3: boolean };
  keypadPowered: boolean;
  cellarOpen: boolean;
  mainOpen: boolean;
  alarmActive: boolean;
  onAlarm: any;

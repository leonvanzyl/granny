...
import { createRenderCore } from './render.js';
import { PhysicsWorld } from './physics.js';
import { buildWorld } from './world.js';
import { Player } from './player.js';
import { Granny } from './granny.js';
import { Puzzle } from './items.js';
import { AudioEngine } from './audio.js';
import { UI } from './ui.js';
import { Input } from './input.js';
import { Touch } from './touch.js';
import { GRANNY, AUDIO, DIFFICULTY, GROUP } from './config.js';
import { clamp01, now } from './util.js';
...
if (AudioEngine.setQuality) AudioEngine.setQuality(MOBILE ? 'low' : 'high');
...
function buildRun(seed) {
  ...
  // wiring
  player.onNoise = (evt) => { granny.hear(evt); AudioEngine.playNoise(evt); };
  physics.onImpact = (body, speed, pos) => {
    ...
    if (lure && k === 'item' && granny.stunT <= 0) {
      const dgx = pos.x - granny.body.position.x, dgz = pos.z - granny.body.position.z;
  ...
  puzzle.setGrannyAware(() => granny.awareness);
  puzzle.onAlarm = () => { G.alarm = true; G.alarmT = DIFFICULTY.alarmSeconds; granny.awareness = GRANNY.awarenessMax; granny._enter('chase'); granny.lastKnown = { x: player.body.position.x, z: player.body.position.z }; };
...
function teardown() {
  ...
  for (let i = core.scene.children.length - 1; i >= 0; i--) {
    const c = core.scene.children[i];
    if (c.isLight || c === core.camera) continue;
    c.traverse((o) => { if (o.isMesh && o.geometry) o.geometry.dispose(); }); // free GPU geometry (materials are shared & cached)
    core.scene.remove(c);
  }
...
function stepGame(dt, t) {
  {
    ...
    if (AudioEngine.setMenace) AudioEngine.setMenace(prox, aware, chasing);
    if (chasing && !_wasChasing && AudioEngine.dreadDrop) AudioEngine.dreadDrop();
    _wasChasing = chasing;
...
window.GDBG = { G, core, AudioEngine, Input, Touch, MOBILE, manualFrame, stepGame };

...
const S = {
  ...
  // Dread subsystem (Silent Hill / P.T. layer): radio static + metallic groans
  // + whisper/drop one-shots. Built in doUnlock() after buildMusic().
  dread: null,            // see buildDread()
  dreadComp: null,        // dedicated compressor on the dread bus (tames its energy locally)
  dreadQuality: 'high',   // 'high' | 'low' — set via setQuality() before build
  _jumpAt: -99,           // ctx time of last jumpscare (dreadDrop cross-guard)
...
// Uniform random in [a, b). Used by the dread scheduler/one-shots for variety.
function rnd(a, b) {
...
function buildBuses() {
  ...
  const buses = {};
  const names = ['sfx', 'granny', 'heartbeat', 'ambient', 'music', 'ui', 'dread'];
  ...
  // The dread bus gets its OWN gentle compressor BEFORE master so its static /
  // groan / sub-thump energy is tamed locally and never drives the brutal master
  // limiter into low-end pumping (see audio design critique fix #1).
  const D = AUDIO.dread;
  const dreadComp = ctx.createDynamicsCompressor();
  dreadComp.threshold.value = D.compThreshold;
  dreadComp.knee.value = 6;
  dreadComp.ratio.value = D.compRatio;
  dreadComp.attack.value = D.compAttack;
  dreadComp.release.value = D.compRelease;
  buses['dread'].disconnect();        // detach from master (connected in the loop above)
  buses['dread'].connect(dreadComp);
  dreadComp.connect(master);
  S.dreadComp = dreadComp;
...
// ---------------------------------------------------------------------------
// Dread subsystem (Silent Hill / P.T.): radio static driven by a smoothed
// "menace" scalar, an irregular metallic-groan scheduler folded into the
// heartbeat tick, plus whisper / drop one-shots. Scope is deliberately the
// 2-layer subset from the design critique (NO always-on sub-drone / breathing
// bed) to avoid stacking low-end into the master limiter.
// ---------------------------------------------------------------------------

function buildDread() {
  const ctx = S.ctx;
  const D = AUDIO.dread;
  const bus = S.buses.dread;
  const low = S.dreadQuality === 'low';
  const sources = [];
  const allNodes = [];

  // --- Radio static: the SH signature; the ONLY continuously menace-driven layer.
  const staticOut = ctx.createGain(); staticOut.gain.value = 1; staticOut.connect(bus);
  const src = noiseSource();
  const hp = ctx.createBiquadFilter(); hp.type = 'highpass'; hp.frequency.value = D.staticHpHz; // thin transistor hiss
  const lp = ctx.createBiquadFilter(); lp.type = 'lowpass'; lp.frequency.value = Math.max(200, D.staticMinHz); lp.Q.value = 0.7; // brightness = menace
  const sgain = ctx.createGain(); sgain.gain.value = D.staticMinGain; // loudness = menace (starts silent)
  src.connect(hp); hp.connect(lp); lp.connect(sgain); sgain.connect(staticOut);
  sources.push(src); allNodes.push(staticOut, hp, lp, sgain);

  // --- Optional crackle (desktop only): sparse popping, gated by an asymmetric
  // square LFO, riding the menace gain so it only crackles as she nears.
  let crackle = null;
  if (!low) {
    const cn = noiseSource();
    const cvca = ctx.createGain(); cvca.gain.value = 0;
    const clfo = ctx.createOscillator(); clfo.type = 'square'; clfo.frequency.value = 11;
    const clfoG = ctx.createGain(); clfoG.gain.value = 0.5;
    const cbias = ctx.createConstantSource(); cbias.offset.value = -0.2; // < 0.5 duty -> sparse pops
    const cg = ctx.createGain(); cg.gain.value = 0.3;
    clfo.connect(clfoG); clfoG.connect(cvca.gain); cbias.connect(cvca.gain);
    cn.connect(cvca); cvca.connect(cg); cg.connect(sgain); // ride the menace-driven static gain
    sources.push(cn, clfo, cbias);
    allNodes.push(cvca, clfoG, cg);
    crackle = { cvca };
  }

  const t = ctx.currentTime;
  for (const s of sources) { try { s.start(t); } catch (e) { /* already started */ } }

  S.dread = {
    static: { out: staticOut, lp, gain: sgain },
    crackle,
    menace: 0, menaceTarget: 0, lastSmooth: t,
    groan: { nextTime: t + rnd(2, 5) },
    lastWhisper: -99, lastDrop: -99,
    sources, allNodes,
  };
}

// Scheduled inside heartbeatTick's look-ahead. Spawns irregular metallic groans
// whose rate rises with menace (the house "wakes up" as she closes in).
function scheduleMetallic(scheduleUntil) {
  if (!S.dread || S.dreadQuality === 'low') return; // groans are desktop-only
  const g = S.dread.groan;
  const m = S.dread.menace;
  const range = m < 0.25 ? AUDIO.dread.groanIntervalIdle
    : m < 0.6 ? AUDIO.dread.groanIntervalNear
      : AUDIO.dread.groanIntervalChase;
  while (g.nextTime < scheduleUntil) {
    spawnGroan(g.nextTime, m);
    g.nextTime += rnd(range[0], range[1]);
  }
}

// One metallic groan (80%) or inharmonic clang (20%), non-positional on the
// dread bus. Self-cleans via the last source's onended.
function spawnGroan(t, menace) {
  const ctx = S.ctx;
  const D = AUDIO.dread;
  const gain = D.groanGain * (0.5 + 0.5 * clamp01(menace));
  const out = ctx.createGain(); out.gain.value = 1; out.connect(S.buses.dread);
  const nodes = [out];

  if (Math.random() < 0.2) {
    // CLANG: 4 inharmonic sine partials with fast decay + a noise tick.
    const f = rnd(80, 150);
    ...
    for (const p of partials) {
      ...
      g.gain.linearRampToValueAtTime(gain * (1 / p) * 0.8, t + 0.005);
      expRampTo(g.gain, TINY, t + 0.9);
      o.connect(g); g.connect(out);
      o.start(t); o.stop(t + 0.95);
      nodes.push(o, g); last.push(o);
    }
    const n = noiseSource();
    const nhp = ctx.createBiquadFilter(); nhp.type = 'highpass'; nhp.frequency.value = 3000;
    const ng = ctx.createGain(); ng.gain.setValueAtTime(gain * 0.5, t); expRampTo(ng.gain, TINY, t + 0.05);
    n.connect(nhp); nhp.connect(ng); ng.connect(out);
    n.start(t, Math.random() * (S.noiseBuffer.duration - 0.1)); n.stop(t + 0.08);
    nodes.push(n, nhp, ng);
    last[0].onended = () => disconnectAll(nodes);
  } else {
    // GROAN: two detuned saws -> soft shaper -> bandpass sweeping 180->90Hz.
    const f = rnd(70, 140);
    const shaper = ctx.createWaveShaper(); shaper.curve = makeShaperCurve(3);
    const bp = ctx.createBiquadFilter(); bp.type = 'bandpass'; bp.Q.value = 4;
    bp.frequency.setValueAtTime(180, t); bp.frequency.linearRampToValueAtTime(90, t + 1.6);
    const g = ctx.createGain();
    g.gain.setValueAtTime(TINY, t);
    g.gain.linearRampToValueAtTime(gain, t + 0.25);
    expRampTo(g.gain, TINY, t + rnd(1.4, 2.0));
    shaper.connect(bp); bp.connect(g); g.connect(out);
    const oscs = [];
    for (const det of [rnd(-18, -8), rnd(8, 18)]) {
      const o = ctx.createOscillator(); o.type = 'sawtooth'; o.frequency.value = f; o.detune.value = det;
      o.connect(shaper); o.start(t); o.stop(t + 2.1); oscs.push(o);
    }
    nodes.push(shaper, bp, g, ...oscs);
...
function heartbeatTick() {
  ...
  // dread metallic groans share it too (so the house groans even when calm)
  scheduleMetallic(scheduleUntil);
...
function attachVisibilityHandler() {
  ...
  document.addEventListener('visibilitychange', () => {
    ...
    if (document.hidden) {
      ...
      if (S.dread) { S.dread.groan.nextTime = tnow() + 1; S.dread.lastSmooth = tnow(); }
...
export const AudioEngine = {
  ...
  playSfx(name, pos = null, opts: any = {}) {
  ...
  // Set render quality for the dread layer. 'low' (mobile) drops the crackle
  // sub-layer and skips the metallic-groan scheduler. Stores the flag for
  // buildDread(); if already built, mutes the desktop-only crackle.
  setQuality(q) {
    S.dreadQuality = q === 'low' ? 'low' : 'high';
    if (S.dread && S.dread.crackle && S.dreadQuality === 'low') {
      approach(S.dread.crackle.cvca.gain, 0, 0.1);
    }
  },

  // Per-frame drive for the radio static. prox & aware are 0..1 (computed by the
  // caller); chasing forces a menace floor. Smoothed at audio rate (NOT written
  // raw each frame) to avoid zipper noise. menace also bands the groan rate.
  setMenace(prox, aware, chasing) {
    if (!S.unlocked || !S.dread) return;
    const D = AUDIO.dread;
    let m = clamp01(0.55 * clamp01(aware == null ? 0 : aware) + 0.45 * clamp01(prox == null ? 0 : prox));
    if (chasing) m = Math.max(m, 0.85);
    S.dread.menaceTarget = m;
    const t = tnow();
    const rising = m > S.dread.menace;
    const tc = rising ? D.menaceRiseTC : D.menaceFallTC;
    // smoothed scalar used by the groan scheduler band (frame-rate stepped)
    const dt = Math.max(0, t - S.dread.lastSmooth); S.dread.lastSmooth = t;
    const a = 1 - Math.exp(-dt / Math.max(0.001, tc));
    S.dread.menace += (m - S.dread.menace) * a;
    // params smoothed at audio rate via setTargetAtTime (approach)
    const st = S.dread.static;
    const gTarget = lerp(D.staticMinGain, D.staticMaxGain, m * m); // square -> sudden swell near her
    const lpTarget = Math.max(200, lerp(D.staticMinHz, D.staticMaxHz, m));
    approach(st.gain.gain, gTarget, tc, t);
    approach(st.lp.frequency, lpTarget, tc, t);
  },

  // P.T. whisper sting: a sibilant sweep panned to Granny's bearing. Debounced.
  whisper(pos) {
    if (!S.unlocked || !S.ctx || !S.dread) return;
    const now = tnow();
    if (now - S.dread.lastWhisper < AUDIO.dread.whisperDebounce) return;
    S.dread.lastWhisper = now;
    const ctx = S.ctx;
    const tr = beginTransient('dread', asVec(pos), AUDIO.dread.whisperGain);
    const t0 = tr.t0;
    const n = noiseSource();
    const bp = ctx.createBiquadFilter(); bp.type = 'bandpass'; bp.Q.value = 6;
    bp.frequency.setValueAtTime(1200, t0); bp.frequency.linearRampToValueAtTime(2400, t0 + 0.6); // the 'sss'
    const g = ctx.createGain();
    g.gain.setValueAtTime(TINY, t0);
    g.gain.linearRampToValueAtTime(AUDIO.dread.whisperGain, t0 + 0.08);
    expRampTo(g.gain, TINY, t0 + 0.7);
    // 4Hz tremolo VCA for syllable feel
    const trem = ctx.createOscillator(); trem.type = 'sine'; trem.frequency.value = 4;
    const tremG = ctx.createGain(); tremG.gain.value = 0.5;
    const tremBias = ctx.createConstantSource(); tremBias.offset.value = 0.5;
    const tremVca = ctx.createGain(); tremVca.gain.value = 0;
    trem.connect(tremG); tremG.connect(tremVca.gain); tremBias.connect(tremVca.gain);
    n.connect(bp); bp.connect(tremVca); tremVca.connect(g); g.connect(tr.out);
    tr.voice.nodes.push(n, bp, g, trem, tremG, tremBias, tremVca);
    n.start(t0, Math.random() * (S.noiseBuffer.duration - 0.8)); n.stop(t0 + 0.75);
    trem.start(t0); trem.stop(t0 + 0.75); tremBias.start(t0); tremBias.stop(t0 + 0.75);
    n.onended = () => finishTransient(tr.voice);
  },

  // The P.T. "drop": yank all beds into a brief vacuum, then restore, with a
  // sub-thump punching through (routed past the bus duck via the dread comp).
  // Debounced, and skipped while a jumpscare is in flight (they fight over gains).
  dreadDrop() {
    if (!S.unlocked || !S.ctx || !S.dread) return;
    const ctx = S.ctx;
    const t0 = ctx.currentTime;
    const D = AUDIO.dread;
    if (t0 - S.dread.lastDrop < D.dropDebounce) return;
    if (t0 - (S._jumpAt || -99) < D.jumpGuardSec) return; // jumpscare cross-guard
    S.dread.lastDrop = t0;

    const duckBuses = ['ambient', 'music', 'dread', 'sfx', 'heartbeat'];
    for (const b of duckBuses) {
      ...
      g.gain.linearRampToValueAtTime(base * D.dropDuckTo, t0 + 0.04);
      g.gain.setValueAtTime(base * D.dropDuckTo, t0 + D.dropHold);
      g.gain.linearRampToValueAtTime(base, t0 + D.dropHold + D.dropRelease);
    }

    // deep sub-thump (pressure hit) routed to the dread COMPRESSOR, bypassing the
    // ducked dread bus gain so it punches through the vacuum but stays tamed.
    const o = ctx.createOscillator(); o.type = 'sine';
    o.frequency.setValueAtTime(40, t0); expRampTo(o.frequency, 28, t0 + 0.8);
    const g = ctx.createGain(); g.gain.setValueAtTime(0.6, t0); expRampTo(g.gain, TINY, t0 + 0.8);
    o.connect(g);
    if (S.dreadComp) g.connect(S.dreadComp); else g.connect(S.master || ctx.destination);
  ...
  jumpscare() {
    ...
    const ctx = S.ctx;
    const t0 = ctx.currentTime;
    S._jumpAt = t0; // mark in-flight so a coincident dreadDrop() backs off (cross-guard)

    // duck buses briefly, then restore ('dread' included so its bed ducks too)
    const duckBuses = ['ambient', 'music', 'heartbeat', 'sfx', 'dread'];
    for (const b of duckBuses) {
      const g = S.buses[b];
      if (!g) continue;
      const base = AUDIO.busGains[b] != null ? AUDIO.busGains[b] : 0.8;
  ...
  stopAll(fadeMs = 80) {
    ...
    const teardown = () => {
      ...
      if (S.dread) stopList.push(...S.dread.sources);
      ...
      if (S.dread) disconnectAll(S.dread.allNodes);
      if (S.dreadComp) disconnectAll([S.dreadComp]);
      // disconnect transients
      for (const v of S.transients) finishTransient(v);
      S.transients = [];
      S.granny = null; S.music = null; S.ambientNodes = null;
      S.dread = null; S.dreadComp = null; S._jumpAt = -99;
      S.unlocked = false;

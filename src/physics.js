...
import { PHYS, GROUP, CHAR } from './config.js';
...
export class PhysicsWorld {
  ...
  addFurnitureBox(size, pos, mass, mesh, opts = {}) {
    ...
    body.userData = { kind: 'furniture', maxSpeed: PHYS.charMaxSpeed, mesh };
    this.world.addBody(body);
  ...
  addItemBox(size, pos, mass, mesh) {
    ...
    body.userData = { kind: 'item', maxSpeed: PHYS.itemMaxSpeed, mesh };
    this.world.addBody(body);
    this.track(mesh, body);
    this.dynamicBodies.push(body);
    this.movableBodies.push(body);
  ...
  addCharacter(pos, group) {
    ...
    body.userData = { kind: group === GROUP.GRANNY ? 'granny' : 'player', maxSpeed: PHYS.charMaxSpeed };
    this.world.addBody(body);
    this.dynamicBodies.push(body);
    this.tracked.push({ mesh: null, body, prev: { x: pos[0], y: pos[1], z: pos[2] }, prevQ: { x: 0, y: 0, z: 0, w: 1 } });
    body._trackRef = this.tracked[this.tracked.length - 1];
    return body;
  }
  ...
  addDoor(opts) {
    ...
    leaf.userData = { kind: 'door', maxSpeed: PHYS.charMaxSpeed, mesh };
  ...
  _clampSpeeds() {
    for (const b of this.dynamicBodies) {
      ...
      const max = (b.userData && b.userData.maxSpeed) || PHYS.charMaxSpeed;
      const v = b.velocity;

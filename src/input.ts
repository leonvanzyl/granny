...
export const Input = {
  ...
  init(canvas: HTMLCanvasElement, opts: { onLockChange?: (locked: boolean) => void } = {}) {
    this.canvas = canvas;
  ...
  down(code: string) { return this.keysDown.has(code); },
  pressed(code: string) { return this._pressed.has(code); },
